...
interface LiskL1DeployScriptParams {
  deployer: Wallet;
  admins: { proxy: string; bridge: string };
}

interface LiskL2DeployScriptParams extends LiskL1DeployScriptParams {
  l2Token?: { name?: string; symbol?: string };
}

interface LiskDeploymentOptions extends CommonOptions {
  logger?: Logger;
  overrides?: Overrides;
}
...
export default function deployment(
  ...
  options: LiskDeploymentOptions = {}
) {
  const liskAddresses = addresses(networkName, options);
  return {
    async erc20TokenBridgeDeployScript(
      l1Token: string,
      l1Params: LiskL1DeployScriptParams,
      l2Params: LiskL2DeployScriptParams
    ) {
      const [
