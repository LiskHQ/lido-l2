...
import {
  ...
  OptimismBridgeExecutor__factory, //FIXME and copy paste to Lisk factory
...
scenario("Lisk :: Bridge Executor integration test", ctxFactory)
...
async function ctxFactory() {
  ...
  const [, l2DeployScript] = await lisk
  ...
  const liskContracts = lisk.contracts(networkName, { forking: true });

  const l1CrossDomainMessengerAliased = await testing.impersonate(
    testing.accounts.applyL1ToL2Alias(
      liskContracts.L1CrossDomainMessenger.address
    ),
    l2Provider
  );

  const l2CrossDomainMessenger =
    await liskContracts.L2CrossDomainMessenger.connect(
